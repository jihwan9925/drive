...
<body>
	...
	<script>
		$("#uploadBtn").click(e=>{
			...
			$.ajax({
				...
				data:form,
				//기본 form방식이 아닌 멀티파트form형식으로 변경시 사용
				type:"post",
				processData:false,
				contentType:false,
				success:data=>{
	...
	$("#upfiles").change(e=>{
		...
		$("#profileInput").change(e=>{
			const reader=new FileReader();
			reader.onload=e=>{ //.onload : 정상적으로 로딩이 되면 실행하게 만드는 로직
				//e.target.result속성에 변경된 file이 나온다.
				$("#profile").attr("src",e.target.result);
			}
			//.readerAsDataURL : 보안을 위해 막아둔 경로를 받아오는 역할을 한다 (현재 선택된 파일의 첫번째파일경로)
